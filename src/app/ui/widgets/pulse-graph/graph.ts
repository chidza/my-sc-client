namespace mrs.ui {
    ...
    interface IPulseGraph extends ng.IController {

    }

    class Controller implements IPulseGraph {
        ...
        ActivePhasePulse: Array<data.ILaborActivePhaseVitalList>;
                ...
                this.deliveryService.getActivePhase(this.deliveryId).then((response) => {
                    ...
                    if (this.activePhaseDateTimes) {
                        ...
                        if (this.dateStartActive) {
                            /* this.getPulseForActiveLabourStageService.getVitalIntervals(this.deliveryId, this.personId, this.vitalId, this.dateStartActive, this.dateEndActive).then((response) => {
                                 // got vitals now i have to package them in arrays by name: 
                              //   this.orderPulseByName(response);
                             });*/
                        }
                    }
                });
        ...
        orderPulseByName = (vitals: Array<data.ILaborActivePhaseVitalList>) => {
                ...
                this.unitService.get(response.unitId).then((unitresponse) => {
                    ...
                    for (let key in vitals) {
                        if (vitals.hasOwnProperty(key)) {
                            ...
                            if (vitals[key]) {
                                this.vitalListTime.push(moment(vitals[key].date).format("HH:mm"));
    ...
    class Component implements ng.IComponentOptions {
        ...
        constructor(
            public templateUrl = "app/ui/widgets/pulse-graph/graph.html",
    ...
    app.component("mrsPulseGraph", new Component());
