namespace mrs.ui {
  ...
  interface IEncounterMedicationList extends ng.IController {
    ...
    deleteMedication: (encounterMedicationId: Object) => void;
  }
  ...
  class Controller implements IEncounterMedicationList {
    ...
    encounterId: string;
    personId: string;
    dispenseMedication: Array<data.IEncounterPrescriptionList>;
    ...
    public deleteMedication: (encounterMedicationId: Object) => void;
    ...
    init = () => {
      ...
      if (this.personId) {
        this.consultationService.getDispenseMedication(this.workspaceId, this.personId).then((response) => {
          this.dispenseMedication = response;
        });
    ...
    deleting = (entry: data.IEncounterPrescriptionList) => {
      this.deleteMedication({ id: entry.dispenseId });
    }
    ...
    delete = (entry: data.IEncounterPrescriptionList) => {
      ...
      dlg.result.then((btn) => {
        ...
        this.prescriptionService.remove(entry.id).then((response) => {
          this.dispenseMedication.splice(this.dispenseMedication.indexOf(entry), 1);
          this.refreshed();
        });
      }, (error) => {
      });
  ...
  class Component implements ng.IComponentOptions {
    ...
    constructor(
      public templateUrl = "app/ui/widgets/encounter-dispensed-prescription/list.html",
  ...
  app.component("mrsEncounterDispenseMedicationList", new Component());
